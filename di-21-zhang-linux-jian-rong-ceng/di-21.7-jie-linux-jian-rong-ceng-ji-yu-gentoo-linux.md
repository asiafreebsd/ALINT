...
```sh
# wget https://mirrors.ustc.edu.cn/gentoo/releases/amd64/autobuilds/20230101T164658Z/stage3-amd64-systemd-20230101T164658Z.tar.xz  # 下载 Gentoo stage3 镜像，链接随版本更新，请读者自行查找最新链接
# mkdir -p /compat/gentoo        # 创建 Gentoo 兼容层目录
# tar xpvf stage3-amd64-systemd-20230101T164658Z.tar.xz -C /compat/gentoo --numeric-owner  # 解压 Gentoo 镜像到兼容层
```
...
FEATURES="-ipc-sandbox -mount-sandbox -network-sandbox -pid-sandbox -xattr -sandbox -usersandbox"  # 禁用 Gentoo 编译系统中的各类沙箱功能
```

进行常见配置：

```sh
# mkdir -p /compat/gentoo/etc/portage/repos.conf/                                        # 创建 Gentoo Portage 仓库配置目录，此时操作在 FreeBSD
# cp /compat/gentoo/usr/share/portage/config/repos.conf /compat/gentoo/etc/portage/repos.conf/gentoo.conf  # 复制默认仓库配置文件，此时操作在 FreeBSD
# cp /etc/resolv.conf /compat/gentoo/etc/                                                   # 复制 DNS 配置到 Gentoo 兼容层，此时操作在 FreeBSD
```
...
tar zxvf stage3-amd64-systemd*.tar.xz -C ${rootdir} --numeric-owner   # 解压 Stage3 文件
