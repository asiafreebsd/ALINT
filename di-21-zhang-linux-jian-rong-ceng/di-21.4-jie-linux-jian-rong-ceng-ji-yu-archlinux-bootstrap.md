...
对于滚动发行版，Linux 兼容层默认内核通常较低。直接构建的话，Arch 兼容层会在 chroot 的时候报错 `FATAL: kernel too old`。需要将 Linux 兼容层的内核版本改为 6.12.63（或其他较高版本）才可以。
...
```sh
# echo "compat.linux.osrelease=6.12.63" >> /etc/sysctl.conf # 持久性设置
# sysctl compat.linux.osrelease=6.12.63 # 立刻生效
```
...
case $answer in
	[Nn][Oo]|[Nn])
		...
		;;
	[Yy][Ee][Ss]|[Yy]|"")
		echo "compat.linux.osrelease=6.12.63" >> /etc/sysctl.conf
		sysctl compat.linux.osrelease=6.12.63
                ;;
esac
echo "complete!"
